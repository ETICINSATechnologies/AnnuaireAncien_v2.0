...
class PositionForm extends Component<PositionFormProps, PositionFormState> {
      ...
      .then(result => {
        this.setState({
          ...
          idMapping: result.reduce(
            (currentMapping: object, position: Position, index: number) => {
              return {
                ...currentMapping,
                [position.id]: index
              };
  ...
  addPosition() {
    ...
    if (mPositionNb < 4) {
      ...
      this.props.updateMemberPositions([
        ...this.props.memberPositions,
        lastPosition
      ]);
    }
  }
  ...
  renderPositions() {
    if (this.props.memberPositions.length !== 0) {
      return this.props.memberPositions.map((mPosition, index) => {
        return (
    ...
    return (
      <div className="position_form">
        ...
        <input
          ...
          className={`input_button add_position ${
            this.props.modifyEnabled ? "visible" : "hidden"
          }`}
        />
      </div>
    );
...
const MemberPositionRender = (props: PositionProps) => {
  return (
    <div className="position_container">
      ...
      <p>Année</p>
      <img
        className={`delete_position ${
          props.modifyEnabled ? "visible" : "hidden"
        }`}
        src={deleteIcon}
        alt="Supprimer"
        onClick={() => props.deletePosition(props.index)}
      />
      <input
        className="isBoard"
        type="checkbox"
        checked={props.mPosition.isBoard}
        onChange={(evt: React.ChangeEvent) => props.onChange(evt, props.index)}
        disabled={!props.modifyEnabled}
      />
      <DropDown
        className="position"
        isRequired={true}
