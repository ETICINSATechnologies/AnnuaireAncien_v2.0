...
class MemberArray extends Component<MemberArrayProps, MemberArrayState> {
  ...
  getPageSize = () => {
    if (window.matchMedia("(orientation:landscape)")) {
      ...
      window.matchMedia("(orientation:portrait) and (max-width: 500px)")
    ) {
      return 15;
    } else {
      return 8;
  ...
  selectMemberById = (id: number) => {
    this.props.selectMember(
      this.state.members[(this.state.memberMapping as any)[id]]
    );
  };
  ...
  render() {
    ...
    if (this.state.status === "result") {
      ...
      if (this.state.status === "init")
        message =
          "Vous pouvez effectuer une recherche sur les critères ci-contre";
      else if (this.state.status === "noResult")
        ...
        <div className="searchResult">
          ...
          {this.state.page.totalPages === 1 ? null : (
            <div className="pagination">
              <input
                disabled={this.state.page.currentPage === 0}
                type="button"
                className="pagePrevious"
                value="Page précédente"
                onClick={() =>
                  this.props.getPage(this.state.page.currentPage - 1)
                }
              />
              ...
              <p className="pageIndicator">
                ...
                <input
                  type="number"
                  className="pageInput"
                  value={this.state.bufferInput}
                  onKeyDown={this.selectPageByInput}
              ...
              <input
                disabled={
                  this.state.page.currentPage + 1 === this.state.page.totalPages
                }
                type="button"
                className="pageNext"
                value="Page suivante"
